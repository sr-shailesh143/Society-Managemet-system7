  ...
  const handleAddOrUpdateVisitor = async () => {
    try {
      if (isEditing) {
        ...
        setVisitors((prev) =>
          prev.map((visitor) =>
            visitor._id === editingVisitorId ? { ...visitor, ...newVisitor } : visitor
          )
        );
      } else {
        const response = await createVisitor(newVisitor);
    ...
    <table className="table table-borderless align-middle">
      <thead>
        <tr>
          ...
          <th className="p-3" style={{ backgroundColor: '#E5ECFD', borderRadius: '0px 15px 0px 0px' }}>Time</th>
        </tr>
      </thead>
      <tbody>
