...
exports.addresidentData = async (req, res) => {
  try {
    ...
    const Adhar_front_side = req.files.Adhar_front_side
      ? await uploadImageToCloudinary(req.files.Adhar_front_side[0].path)
      : null;
    const Adhar_back_side = req.files.Adhar_back_side
      ? await uploadImageToCloudinary(req.files.Adhar_back_side[0].path)
      : null;
    const Address_proof = req.files.Address_proof
      ? await uploadImageToCloudinary(req.files.Address_proof[0].path)
      : null;
    const Rent_Agreement = req.files.Rent_Agreement
      ? await uploadImageToCloudinary(req.files.Rent_Agreement[0].path)
      : null;
